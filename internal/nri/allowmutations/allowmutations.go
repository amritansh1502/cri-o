...
// ValidateContainerAdjustment logs a warning for containers whose pod
// namespace is not in the configured allowlist, but allows them through.
func (p *Plugin) ValidateContainerAdjustment(ctx context.Context, req *api.ValidateContainerAdjustmentRequest) error {
	...
	logrus.Warnf("AllowMutations: namespace %q is not in the allowed list for %s/%s (allowing anyway)",
		namespace, podName, containerName)
	return nil
}
